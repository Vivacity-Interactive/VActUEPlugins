...
void ABeatManagerActor::BeginPlay()
{
	...
	int32 Index = 0;
	for (const TSubclassOf<UBeatsProfile>& ScenarioClass : ScenarioClasses)
	{
		UBeatsProfile* Scenario = NewObject<UBeatsProfile>(this, ScenarioClass);
		Scenario->InitClaims();
		Scenario->AddToRoot();
...
void ABeatManagerActor::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
	...
	for (UBeatsProfile* Scenario : Scenarios) { if (Scenario) { Scenario->RemoveFromRoot(); Scenario = nullptr; } }
}
