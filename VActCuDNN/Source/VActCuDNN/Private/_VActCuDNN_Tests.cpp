...
void _FVActCuDNNTests::_Debug_Cuda_Test()
{
	const int32 Num = 4;
...
void _FVActCuDNNTests::_Debug_CuDNN_Test_0()
{
...
void _FVActCuDNNTests::_Debug_CuDNN_Test_1()
{
	cudnnHandle_t Handle = nullptr;
...
void _FVActCuDNNTests::_Debug_VActCuDNN_Test_0()
{
	const int32 Width = 4, Height = 4, Size = 1;
	TArray<float> InputData, OutputData;

	FCuDevice Device = FVActCuDNN::CreateDevice(-1);
	UE_LOG(LogTemp, Display, TEXT("VActCuDNN Test 0 Using Device %s(%d, %d)"), ANSI_TO_TCHAR(Device.Specs.name), Device.Index, Device.Id);
	FVActCuDNN::Use(Device);

	FCuContext Context = FVActCuDNN::CreateContext();
	// TODO Find Way To Use TArray as data
	FCuTensor Input = FVActCuDNN::CreateTensor(Width, Height, Size);
	FCuTensor Output = FVActCuDNN::CreateTensor(Width, Height, Size);
	FCuActivation Activation = FVActCuDNN::CreateActivation(Input, Output);

	FVActCuDNN::Forward(Context, Activation, Input, Output);

	FVActCuDNN::Destroy(Input);
	FVActCuDNN::Destroy(Output);
	FVActCuDNN::Destroy(Activation);
	FVActCuDNN::Destroy(Context);

	UE_LOG(LogTemp, Display, TEXT("VActCuDNN Test 0 Success"));
}

void _FVActCuDNNTests::_Debug_VActCuDNN_Test_1()
{
	...
	FCuTensor Input = FVActCuDNN::CreateTensor(Width, Height, Size);
	FCuTensor Output = FVActCuDNN::CreateTensor(Width, Height, Size);
	FCuKernel Kernel = FVActCuDNN::CreateKernel(Size, Size, Size, Size);
	FCuConvolution Convolution = FVActCuDNN::CreateConvolution();
	FCuAlgorithm Algorithm = FVActCuDNN::CreateAlgorithm(Context, Input, Kernel, Convolution, Output, Result);
	...
	FCuInstance Instance = FVActCuDNN::CreateInstance(Context, Input, Kernel, Convolution, Output, Algorithm, KernelData);

	//FVActCuDNN::Use(Context, Alpha, Beta, Input, InputData, Kernel, Convolution, Output, Algorithm, InputData, OutputData, KernelData);
	//FVActCuDNN::Use(Instance);

	FVActCuDNN::Destroy(Kernel);
	FVActCuDNN::Destroy(Input);
	FVActCuDNN::Destroy(Output);
	FVActCuDNN::Destroy(Convolution);
	FVActCuDNN::Destroy(Algorithm);
	FVActCuDNN::Destroy(Result);
	//FVActCuDNN::Destroy(Workspace);

	//FVActCuDNN::Destroy(Instance);
